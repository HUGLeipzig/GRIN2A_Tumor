...
### Cosmic
- Downloaded the GRCh37 VCF Files (coding and non-coding mutations) from https://cancer.sanger.ac.uk/cosmic/download
- Indexed the VCF with `tabix -p vcf CosmicCodingMuts.vcf.gz` in WSL2
- Extracted the Region with GRIN2A based on Emsembl Coordinates with `tabix CosmicCodingMuts.vcf.gz 16:9,847,261-10,276,785 > 02_Cosmic_GRIN2A.vcf`
- Downloaded the Metadata (for sample information etc) from COSMIC Mutation Data --> Filter by Gene: GRIN2A
...
First read the data files. The .vcf files have to be edited a little bit to extract
the metadata. Filter the files for *GRIN2A* and/or the right transcript. 
We also filter the **Cosmic** dataset for **confirmed somatic** variants.
In **ClinVar** we **exclude benign and somatic** variants.

```{r Read and Prepare Data, message=FALSE, results='hide', warning=FALSE}
...
Now we have variants per dataset, but all have different forms of annotation. 

So we **generate a dummy vcf** and **annotate with Ensembl VEP**.

```{r Build variant table and vcf, warning=FALSE}
...
**_Paste this data to the online Ensembl VEP and download the results_, filter for transcript NM_001134407.3**.

https://grch37.ensembl.org/Homo_sapiens/Tools/VEP

Parameters: RefSeq Transcripts, include HGVS. 
Save in folder 01_Ensembl_annotation/02_EnsemblVEP_output. 

## Edit VEP output

The output files from the VEP need to be edited a little bit

```{r Edit VEP output .vcf, warning=FALSE, message=FALSE, results='hide'}
...
```{r Read Domain Data, message=FALSE, results='hide'}
...
We need to know where the variants are located. Do we see a hotspot?
Do somatic variants cluster differently?

First we write a plot function and then plot the data for:
1. All variants
2. missense only
3. Null variants only
...
#### All variants
```{r Print all variants, echo=FALSE, fig.width=12, fig.height=6.75, warning=FALSE}
g_density
ggsave("02_Plots/VariantDistribution_all.png", g_density, device = "png", 
       width = 16*0.75, height = 9*0.75, units = "in")
```

#### Missense variants
```{r Print missense variants, echo=FALSE, fig.width=12, fig.height=6.75, warning=FALSE}
g_density_missense
ggsave("02_Plots/VariantDistribution_missense.png", g_density_missense, device = "png", 
       width = 16*0.75, height = 9*0.75, units = "in")
```

#### Null variants
```{r Print null variants, echo=FALSE, fig.width=12, fig.height=6.75, warning=FALSE}
g_density_null
ggsave("02_Plots/VariantDistribution_null.png", g_density_null, device = "png", 
       width = 16*0.75, height = 9*0.75, units = "in")
```

#### Combined plot
```{r Print combined density, echo=FALSE, fig.width=12, fig.height=13.5, warning=FALSE}
g_density_combi
ggsave("02_Plots/VariantDistribution_combi.png", g_density_combi, device = "png", 
       width = 16*0.75, height = 9*0.5*3, units = "in")
```

### Venn diagramm

With a Venn diagramm we can visualise which variants are present in which dataset

```{r Venn diagramm, warning=FALSE, message=FALSE, results='hide'}
...
```{r Print Venn diagramm, echo=FALSE, fig.width=6.75, fig.height=6.75}
v
ggsave("02_Plots/Venn.png", v, device = "png", width = 9*0.75, height = 9*0.75, units = "in")
```

### Domain data

We can also incorporate the domain data.
This way we can determine if specific variant types are more frequent in different domains.

#### Prepare domain dataset

First we need to prepare the dataset by assigning the correct domain to every variant

```{r Prepare domain dataset, warning=FALSE, message=FALSE, results='hide'}
...
This dataset can now be used to plot the variant types per domain.
We will use pie charts and split them by domain and dataset, and annotate with the variant consequence.

```{r Plot domain diagramms, warning=FALSE, message=FALSE, results='hide'}
...
```{r Print domain diagramms, echo=FALSE, fig.height=12, fig.width=6.75}
g_domain
ggsave("02_Plots/Domain_PieCharts.png", g_domain, device = "png", width = 9*0.75, height = 16*0.75, units = "in")
```


## Statistics

Here we validate our observations with statistical analyses. 
In particular, we test for a uniform variant distribution with the Kolmogorov Smirnov Test 
...
```{r Print Statistics diagramms, echo=FALSE, fig.height=9, fig.width=16}
combi
ggsave(plot = combi, filename = "02_Plots/KS_Plots_combined.png", 
